...
@pytest.fixture
def ships_tap():
    return ShipsTap(base_url="https://api.spacexdata.com/v4/", config_path="config_snowflake.json")
...
def test_fetch_ships_malformed_response():
    ...
    with patch('requests.get') as mock_get:
        ...
        incomplete_response.json.return_value = [{"name": "Test Ship"}]  # Missing required 'id' field
...
def test_fetch_ships_multiple_types():
    ...
    with patch('requests.get') as mock_get, \
        ...
        assert json.loads(records[0]['ROLES']) == ["Fairing Recovery", "Dragon Recovery"]
