...
def test_fetch_crew_successful():
    ...
    with (
        ...
        assert json.loads(first_record["record"]["LAUNCHES"]) == [
...
def test_fetch_crew_missing_fields():
    ...
    with (
        ...
        assert (
            record_call["record"]["LAUNCHES"] == "[]"
        )  # Empty array for missing launches
...
def test_fetch_crew_state_management():
    ...
    with (
        ...
        mock_get_state.return_value = {
            "bookmarks": {"crew": {"last_record": "2024-01-01T00:00:00Z"}}
        }
