{{
    config(
        ...
        unique_key='bridge_launch_core_key'
    )
}}
...
with launch_cores as (
    select
        ...
        launch_created_at as launch_created_at

    from {{ ref('stg_spacex_data__launch') }} ,
        lateral flatten(input => launch_core_serial_numbers) as launch_cores
)

select
    {{ dbt_utils.generate_surrogate_key(['launch_cores.launch_cores_launch_id', 'cores.core_id']) }} as bridge_launch_core_key,
    ...
    launch_cores.launch_created_at as bridge_launch_core_created_at

from launch_cores
    inner join {{ ref('stg_spacex_data__core') }} cores
        on launch_cores.launch_cores_id = cores.core_id

{% if is_incremental() %}
    where launch_cores.launch_created_at > (select max(bridge_launch_core_created_at) from {{ this }})
{% endif %}
